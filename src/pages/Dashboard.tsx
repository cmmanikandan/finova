  ...
  return (
    <div className="page-enter" style={{ display: 'flex', flexDirection: 'column', minHeight: '100%' }}>
      ...
      <div style={{
        ...
        flexDirection: 'column',
        gap: '24px',
        paddingBottom: '120px', // clear bottom nav + FAB
      }}>

        {/* 2. Balance Card (~200px height, perfectly aligned) */}
        <div className="balance-gradient" style={{
          ...
          <div>
            <p style={{ margin: 0, fontSize: '0.75rem', opacity: 0.8, fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase' }}>Total Balance</p>
            <h2 style={{ margin: '4px 0 0', fontSize: '2.125rem', fontWeight: 800, letterSpacing: '-0.5px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginTop: '16px' }}>
            {[
              { label: 'Income',  value: stats.income,   icon: <TrendingUp size={12} />,  color: '#4ADE80' },
              { label: 'Expense', value: stats.expense,  icon: <TrendingDown size={12} />, color: '#F87171' },
              { label: 'Savings', value: stats.savings,  icon: <TrendingUp size={12} />,  color: '#60A5FA' },
            ].map(item => (
              <div key={item.label} style={{ background: 'rgba(255,255,255,0.12)', borderRadius: '14px', padding: '10px 8px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginBottom: '2px', color: item.color }}>
                  {item.icon}
                  <span style={{ fontSize: '0.625rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.3px' }}>{item.label}</span>
                </div>
                <div style={{ fontSize: '0.8125rem', fontWeight: 800 }}>{formatCurrency(item.value)}</div>
              </div>
            ))}
          </div>
        </div>

        {/* 3. Quick Actions (Equal width & height 3x2 responsive grid) */}
        <div>
          <h3 style={{ margin: '0 0 10px', fontSize: '0.875rem', fontWeight: 800, color: 'var(--color-text)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Quick Actions</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px' }}>
            ...
            <QuickAction icon={<Target size={20} />}         label="Add Goal"   color="#F59E0B" bg="rgba(245,158,11,0.1)" onClick={() => setActiveTab('goals')} />
            <QuickAction icon={<Sliders size={20} />}        label="Budgets"    color="#7C3AED" bg="rgba(124,58,237,0.1)" onClick={() => setActiveTab('budgets')} />
            <QuickAction icon={<BarChart2 size={20} />}      label="Reports"    color="#06B6D4" bg="rgba(6,182,212,0.1)" onClick={() => setActiveTab('reports')} />
          </div>
        </div>

        {/* 4. Monthly Summary (2x2 equal sized cards grid) */}
        <div className="card" style={{ padding: '16px' }}>
          <h3 style={{ margin: '0 0 12px', fontSize: '0.875rem', fontWeight: 800, color: 'var(--color-text)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Monthly Summary</h3>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <div style={{ background: 'var(--color-bg)', borderRadius: '16px', padding: '12px', border: '1px solid var(--color-border)' }}>
            ...
            <div style={{ background: 'var(--color-bg)', borderRadius: '16px', padding: '12px', border: '1px solid var(--color-border)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        ...
        <div className="card" style={{ padding: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <h3 style={{ margin: 0, fontSize: '0.875rem', fontWeight: 800, color: 'var(--color-text)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Recent Transactions</h3>
            <button
          ...
          {recentTxns.length === 0 ? (
            <div className="empty-state" style={{ padding: '20px 0 10px' }}>
              ...
              <button className="btn-primary" style={{ padding: '8px 16px', fontSize: '0.8125rem' }} onClick={() => openForm('expense')}>
                + Add Transaction
              </button>
            </div>
          ) : (
                ...
                return (
                  <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: '0.8125rem', fontWeight: 700, color: 'var(--color-text)', textTransform: 'capitalize' }}>{cat?.name || t.category}</div>
                      <div style={{ fontSize: '0.6875rem', color: 'var(--color-text-muted)' }}>{formatDate(t.date)} · {formatTime(t.date)}</div>
                    </div>
                    <div style={{
        ...
        {goals.length > 0 && (
          <div className="card" style={{ padding: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <h3 style={{ margin: 0, fontSize: '0.875rem', fontWeight: 800, color: 'var(--color-text)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Active Goals</h3>
              <button
        ...
        {budgets.length > 0 && (
          <div className="card" style={{ padding: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
              <h3 style={{ margin: 0, fontSize: '0.875rem', fontWeight: 800, color: 'var(--color-text)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Budgets</h3>
              <button
